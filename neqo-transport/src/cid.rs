...
impl<SRT: Clone + PartialEq> ConnectionIdStore<SRT> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &ConnectionIdEntry<SRT>> {
        self.cids.iter()
    }
}
...
impl ConnectionIdManager {
    ...
    /// Iterate over all local connection IDs (for migration state export).
    pub fn local_cids(&self) -> impl Iterator<Item = &ConnectionIdEntry<()>> {
