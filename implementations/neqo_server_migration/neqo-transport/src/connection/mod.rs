...
impl Connection {
    ...
    /// Create a server Connection from imported migration state.
    /// This reconstructs enough state for the connection to handle
    /// PATH_CHALLENGE and continue serving traffic after migration.
    pub fn new_from_migration<A1: AsRef<str>, A2: AsRef<str>>(
        certs: &[A1],
        protocols: &[A2],
        cid_generator: Rc<RefCell<dyn ConnectionIdGenerator>>,
        conn_params: ConnectionParameters,
        migration: &crate::migration_state::MigrationState,
    ) -> Res<Self> {
        ...
        conn.crypto
            .crypto_states_mut()
            .import_app_crypto(&migration.write_crypto, &migration.read_crypto)?;

        // Set version
        conn.version = migration.version;

        // Set connection state to Confirmed (handshake is done)
        conn.state = State::Confirmed;

        // Set our local CID from the migration state
        if let Some(local_cid) = migration.local_cids.first() {
            let cid = ConnectionId::from(&local_cid.cid[..]);
            conn.local_initial_source_cid = cid;
        }

        // Add remote (client's) CIDs
        for remote_cid in &migration.remote_cids {
            let cid = ConnectionId::from(&remote_cid.cid[..]);
            let entry = ConnectionIdEntry::new(
                remote_cid.seqno,
                cid,
                Srt::random(),
            );
            conn.cids.add_remote(entry)?;
        }

        // Set up the network path to the client
        let local_addr = SocketAddr::new(
            std::net::IpAddr::V4(std::net::Ipv4Addr::UNSPECIFIED),
            4433,
        );
        let now = Instant::now();
        conn.paths.find_path(
            local_addr,
            migration.client_addr,
            &conn.conn_params,
            now,
            &mut conn.stats.borrow_mut(),
        );
    ...
    fn next_delay(&mut self, now: Instant, paced: bool) -> Duration {
        ...
        let earliest = delays.into_iter().min().unwrap_or(now + Duration::from_secs(5));
...
impl Connection {
    /// Export the connection's migration state for transfer to another server.
    /// This captures the crypto keys, connection IDs, and client address needed
    /// for the preferred server to handle the migrated connection.
    pub fn export_migration_state(&self) -> Res<crate::migration_state::MigrationState> {
        use crate::migration_state::{CidExport, MigrationState};

        // Export crypto state
        let (write_crypto, read_crypto) = self.crypto.crypto_states().export_app_crypto()?;

        // Get client address from primary path
        let primary_path = self.paths.primary().ok_or(Error::NoAvailablePath)?;
        let client_addr = primary_path.borrow().remote_address();

        // Export ALL local CIDs (ours that the client uses as DCID)
        // This includes the preferred address CID (seqno=1)
        let local_cids: Vec<CidExport> = self.cid_manager.local_cids().map(|entry| CidExport {
            cid: entry.connection_id().to_vec(),
            seqno: entry.sequence_number(),
        }).collect();

        // Export remote CIDs (client's CIDs we use as DCID)
        // First, get the CID from the primary path (the one actively in use)
        let mut remote_cids: Vec<CidExport> = Vec::new();
        if let Some(path_cid) = primary_path.borrow().remote_cid() {
            remote_cids.push(CidExport {
                cid: path_cid.to_vec(),
                seqno: 0,
            });
        }
        // Then add any additional CIDs from the store
        for entry in self.cids.iter() {
            let cid_bytes = entry.connection_id().to_vec();
            if !remote_cids.iter().any(|c| c.cid == cid_bytes) {
                remote_cids.push(CidExport {
                    cid: cid_bytes,
                    seqno: entry.sequence_number(),
                });
