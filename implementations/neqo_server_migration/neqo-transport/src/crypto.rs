...
impl Crypto {
    ...
    /// Access the crypto states for migration export.
    pub(crate) fn crypto_states(&self) -> &CryptoStates {
        &self.states
    }

    /// Access the crypto states mutably for migration import.
    pub(crate) fn crypto_states_mut(&mut self) -> &mut CryptoStates {
        &mut self.states
    }
...
pub struct CryptoDxAppData {
    dx: CryptoDxState,
    cipher: Cipher,
    // The secret used to create `self.dx` — retained for migration state export.
    secret: SymKey,
...
impl CryptoDxAppData {
    ...
    /// Access the underlying crypto state for direct packet operations.
    pub fn dx_mut(&mut self) -> &mut CryptoDxState {
        &mut self.dx
    }

    /// Access the underlying crypto state (read-only).
    pub fn dx(&self) -> &CryptoDxState {
        &self.dx
    }

    // --- Migration support ---

    /// Export the crypto state for migration to another server.
    pub fn export_for_migration(&self) -> Res<crate::migration_state::CryptoStateExport> {
        let secret_bytes = self.secret.key_data()?.to_vec();
        let next_secret_bytes = self.next_secret.key_data()?.to_vec();
        Ok(crate::migration_state::CryptoStateExport {
            secret_bytes,
            next_secret_bytes,
            cipher: self.cipher,
            version: self.dx.version,
            direction: self.dx.direction,
            epoch: self.dx.epoch,
            used_pn_start: self.dx.used_pn.start,
            used_pn_end: self.dx.used_pn.end,
            min_pn: self.dx.min_pn,
        })
    }

    /// Reconstruct crypto app data from migration state.
    pub fn new_from_migration(
        ...
        cipher: Cipher,
        epoch: usize,
        used_pn_start: crate::packet::Number,
        used_pn_end: crate::packet::Number,
        min_pn: crate::packet::Number,
        next_secret_bytes: &[u8],
    ) -> Res<Self> {
        let hplabel = String::from(version.label_prefix()) + "hp";
        let next_secret = hkdf::import_key(TLS_VERSION_1_3, next_secret_bytes)?;
        Ok(Self {
            dx: CryptoDxState {
                version,
                direction: dir,
                ...
                aead: Aead::new(TLS_VERSION_1_3, cipher, secret, version.label_prefix())?,
                hpkey: hp::Key::extract(TLS_VERSION_1_3, cipher, secret, &hplabel)?,
                used_pn: used_pn_start..used_pn_end,
                min_pn,
                invocations: CryptoDxState::limit(dir, cipher),
                largest_packet_len: INITIAL_LARGEST_PACKET_LEN,
            },
            cipher,
            secret: secret.clone(),
...
impl CryptoStates {
    ...
    pub(crate) fn test_default() -> Self {
        ...
        let app_read = |epoch| CryptoDxAppData {
            ...
            secret: hkdf::import_key(TLS_VERSION_1_3, &[0xaa; 32]).unwrap(),
    ...
    /// Export application-layer crypto state for migration.
    /// Returns (write_export, read_export) or error.
    pub fn export_app_crypto(&self) -> Res<(
        crate::migration_state::CryptoStateExport,
        crate::migration_state::CryptoStateExport,
    )> {
        let write = self.app_write.as_ref().ok_or(Error::Internal)?;
        let read = self.app_read.as_ref().ok_or(Error::Internal)?;
        Ok((write.export_for_migration()?, read.export_for_migration()?))
    }

    /// Import application-layer crypto state from migration.
    pub fn import_app_crypto(
        &mut self,
        write_state: &crate::migration_state::CryptoStateExport,
        read_state: &crate::migration_state::CryptoStateExport,
    ) -> Res<()> {
        let write_app = crate::migration_state::import_crypto_app_data(write_state)?;
        let read_app = crate::migration_state::import_crypto_app_data(read_state)?;
        let read_next = read_app.next()?;
        self.cipher = write_state.cipher;
        self.app_write = Some(write_app);
        self.app_read_next = Some(read_next);
        self.app_read = Some(read_app);
        Ok(())
    }
}
