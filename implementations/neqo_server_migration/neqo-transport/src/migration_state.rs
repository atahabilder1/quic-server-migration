...
impl MigrationState {
    ...
    pub fn decode(data: &[u8]) -> Res<Self> {
        ...
        // Local CIDs (max 16 per QUIC spec, each max 20 bytes)
        ...
        // Remote CIDs (max 16 per QUIC spec, each max 20 bytes)
    ...
    fn decode_crypto_state(dec: &mut Decoder, version: Version) -> Res<CryptoStateExport> {
        ...
        // Validate secret lengths: 32 bytes for AES-128-GCM/ChaCha20, 48 for AES-256-GCM
        if (secret_bytes.len() != 32 && secret_bytes.len() != 48) || secret_bytes.is_empty() {
            return Err(Error::Internal);
        }
        if (next_secret_bytes.len() != 32 && next_secret_bytes.len() != 48)
            || next_secret_bytes.is_empty()
        {
            return Err(Error::Internal);
        }

        // Validate cipher suite: only TLS 1.3 ciphers allowed
        let cipher: u16 = dec.decode_uint().ok_or(Error::Internal)?;
        let cipher: Cipher = match cipher {
            0x1301 | 0x1302 | 0x1303 => cipher as Cipher, // AES-128-GCM, AES-256-GCM, ChaCha20
            _ => return Err(Error::Internal),
        };
        ...
        // Validate epoch: must be application data epoch (>= 3)
        ...
        // Validate packet number invariants
        if used_pn_start > used_pn_end {
            return Err(Error::Internal);
        }
        if min_pn > used_pn_end {
            return Err(Error::Internal);
        }
