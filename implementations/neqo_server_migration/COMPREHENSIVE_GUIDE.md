...
5. The Architecture: Four Machines
...
Our demo uses four physical machines on the same local network:
...
|  or Firefox      |    |                  |    |                  |
|                  |    |  Modified Neqo   |    |  Custom binary   |
|  No modifications|    |  exports state   |    |  imports state   |
+------------------+    +------------------+    +-------------------+
        |                       |                       |
        |    QUIC (UDP:4433)    |                       |
        |<--------------------->|                       |
...
- The **client** is unmodified -- it's a standard QUIC client (or Firefox browser). It doesn't know that multiple servers are involved.
...
- The **Redis server** runs on a dedicated Proxmox VM, keeping the state transfer infrastructure independent from both QUIC servers.
...
- Redis server on a dedicated machine (141.217.168.200, for redis_kv/redis_ps backends)
- Four machines on the same network: client, primary server, preferred server, Redis server
