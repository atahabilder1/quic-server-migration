...
/// Maximum allowed migration state size (bytes). Real state is ~445 bytes;
/// this cap prevents OOM from a malicious sender claiming a huge length.
const MAX_STATE_SIZE: usize = 10_000;

/// Timeout for TCP read/write operations.
...
impl StateSender for TcpPushSender {
    fn send_state(&mut self, data: &[u8], _instance_id: &str) -> TransferResult<TransferMetrics> {
        ...
        let mut tcp = TcpStream::connect_timeout(&self.addr, TCP_TIMEOUT)?;
...
impl StateReceiver for TcpPushReceiver {
    fn receive_state(&mut self, _instance_id: &str) -> TransferResult<(Vec<u8>, TransferMetrics)> {
        ...
        let listen_addr: SocketAddr = format!("0.0.0.0:{}", self.port).parse()
            .map_err(|e| super::TransferError::Backend(format!("Invalid listen addr: {e}")))?;
        ...
        if data_len > MAX_STATE_SIZE {
            return Err(super::TransferError::Backend(
                format!("State too large: {data_len} bytes (max {MAX_STATE_SIZE})")
            ));
        }
