...
impl StateSender for HttpPullSender {
    fn send_state(&mut self, data: &[u8], _instance_id: &str) -> TransferResult<TransferMetrics> {
        ...
        // Accept one connection (with timeout to prevent indefinite blocking)
...
impl StateReceiver for HttpPullReceiver {
    fn receive_state(&mut self, _instance_id: &str) -> TransferResult<(Vec<u8>, TransferMetrics)> {
        ...
        // Retry connecting until primary's HTTP server is ready (max 50 retries = 5s)
        let mut stream;
                ...
                Err(e) => {
                    ...
                    if retries >= MAX_RETRIES {
                        return Err(TransferError::Backend(
                            format!("Failed to connect to primary after {MAX_RETRIES} retries: {e}")
                        ));
                    }
        ...
        // Read the full response (capped at 64KB to prevent OOM)
            ...
            match stream.read(&mut buf) {
                ...
                Ok(n) => {
                    ...
                    if response.len() > MAX_RESPONSE {
                        return Err(TransferError::Backend(
                            format!("HTTP response too large (>{MAX_RESPONSE} bytes)")
                        ));
                    }
                }
